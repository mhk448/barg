...
class Mailer {
    ...
    public function send() {
        ...
        $boundryMixed = 'ParsAndis-mixed-' . $random_hash;  // Boundry for mixed content type (ie. Text and File Attachments)
        $boundryAlt = 'ParsAndis-alt-' . $random_hash;  // Boundry for Alternative mail types (ie. plain and html)
        // add from
        $from = $this->_from;
    ...
    public function sendMail($to, $subject, $html_body) {
        global $_CONFIGS;
        $this->setBodyHtml($html_body);
        $this->setSubject($subject);
        $this->setFrom($_CONFIGS['Site']['Sub']['Email'], $_CONFIGS['Site']['Sub']['NickPath']);
        $this->addTo($to);
        $this->send();
    }
